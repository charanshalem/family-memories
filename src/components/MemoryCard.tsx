...
export function MemoryCard({ memory, canDelete, onDelete }: Props) {
  return (
    <article className="group relative flex break-inside-avoid flex-col overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-stone-200 transition duration-300 hover:-translate-y-1 hover:shadow-xl">
      ...
      {memory.image_url && (
        <div className="overflow-hidden bg-stone-100 flex justify-center">
          <img
            src={memory.image_url}
            alt={memory.title}
            loading="lazy"
            className="w-full max-h-[700px] object-contain transition duration-500 group-hover:scale-105"
            onError={(e) => {
      ...
      <div className="flex flex-1 flex-col p-6">
        ...
        <p className="mt-2 flex-1 text-sm leading-relaxed text-stone-600 whitespace-pre-wrap">
          {memory.story}
        </p>
